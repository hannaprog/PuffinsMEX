/*
  AUV Main Controller - Arduino Uno
...
const int STEPPER_ENABLE_PIN = 4;   // HIGH only while stepper is actually running
...
// Conservative first-test values. These must be tuned experimentally.
// Thrusters react faster, so they can usually use stronger gains than buoyancy.
const PidGains GAINS_THRUSTER = {40.0, 3.0, 20.0};

// Buoyancy is slower and delayed. Start gentle to avoid overshoot.
const PidGains GAINS_BUOYANCY = {12.0, 0.5, 8.0};
...
// Example: 2 mm/rev, 20 cm stroke -> 100 rev -> 20,000 full steps.
// Adjust to match your real mechanism.
const long PISTON_MIN_STEPS = 0;
const long PISTON_MAX_STEPS = 20000;
const long PISTON_START_STEPS = 10000;  // Start from known middle position

const float STEPPER_U_DEADBAND = 5.0;
...
// ================================================================
// TIMING - CONSERVATIVE FOR ARDUINO UNO
// ================================================================
...
void chooseModeFromSerial() {
  Serial.println();
  Serial.println(F("Select mode:"));
  Serial.println(F("1 = Thrusters"));
  Serial.println(F("2 = Buoyancy / Stepper piston"));
  ...
  if (mode == MODE_THRUSTER) Serial.println(F("THRUSTERS"));
  if (mode == MODE_BUOYANCY) Serial.println(F("BUOYANCY"));
...
void countdownBeforeMission() {
  Serial.print(F("Mission starts in "));
  Serial.print(START_DELAY_SEC);
  Serial.println(F(" seconds."));

  for (unsigned long i = START_DELAY_SEC; i > 0; i--) {
    Serial.print(i);
    Serial.println(F("..."));
    allActuatorsSafe();
    delay(1000);
...
void requestStepperControl(float u) {
  if (abs(u) < STEPPER_U_DEADBAND) {
    stepperShouldRun = false;
    requestedStepperRpm = 0.0;
    return;
  }
  ...
  requestedStepperRpm = clampFloat(abs(u), 0.0, STEPPER_MAX_RPM);
...
void updateControl(unsigned long now_ms) {
  ...
  float u = computePID(targetDepth_m, depth_m, dt_s);
  lastControlOutput = u;

  if (mode == MODE_THRUSTER) {
    applyThrusterControl(u);
  } else if (mode == MODE_BUOYANCY) {
    stopThrusters();
    requestStepperControl(u);
  } else {
    allActuatorsSafe();
  }
}
...
void setup() {
  ...
  sdOK = initSD();
  if (!sdOK) {
    Serial.println(F("WARNING: SD logging disabled. Continuing without SD."));
  }
